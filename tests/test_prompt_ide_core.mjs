...
assert.match(editInstructionTemplate(optionRoomObjectStudy), /equal semantic authority/);
assert.match(editInstructionTemplate(optionRoomObjectStudy), /No picture is the master/);
assert.match(editInstructionTemplate(optionRoomObjectStudy), /Picture 1.*has no priority/);
...
assert.match(source, /function refreshEditContext\(force = false, \{allowTemplatePrompt = true\} = \{\}\)/);
assert.match(source, /const mayPrompt = allowTemplatePrompt && state\.editContextObserved/);
assert.match(source, /state\.editContextObserved \|\|= Boolean\(next\)/);
assert.match(source, /refreshEditContext\(true, \{allowTemplatePrompt:false\}\)/);
...
assert.match(source, /promptRetentionMarkerRanges/);
